      ...
      initial={{opacity:0.2,y:50}} transition={{duration:0.3}} whileInView={{opacity:1,y:0}} viewport={{once:true}}
        >
           ...
           {state!=='Login' &&  <div className='border px-6 py-2 flex items-center gap-2 rounded-full mt-5'>
                ...
                <input onChange={e=>setName(e.target.value)} value={name} className='flex-1 outline-none' type="text" placeholder='Full Name' required />
            </div>}

            <div className='border px-6 py-2 flex items-center gap-2 rounded-full mt-5'>
                <img src={assets.email_icon} alt="" className='w-5 h-5' />
                <input onChange={e=>setEmail(e.target.value)} value={email} className='flex-1 outline-none' type="email" placeholder='Email id' required />
            </div>
            <div className='border px-6 py-2 flex items-center gap-2 rounded-full mt-5'>
                <img src={assets.lock_icon} alt="" className='w-5 h-5' />
                <input onChange={e=>setPassowrd(e.target.value)} value={password} className='flex-1 outline-none' type="password" placeholder='Password' required />
            </div>
